...
// !!! ATTENTION - this generated script is customize - see comment "SL"

(function () {

	/* Include: jQuery (if option selected) */
	!function (a, b) {
	...
	function GenesysChatAPI(e) {
		function n() {
			...
			this.startSession = function (l) {
				var h;
				if (!l.chatServerUrl && !l.transport) {
					a("either chatServerUrl or a custom transport must be provided")
				}
				s = l.logger || t.noop;
				i = o(l);
				h = new e;
				s("startSession: initializing transport");
				i.init().fail(c(h, s)).done(function () {
					var e = {
						username : l.username,
						userData : l.userData,
						source : l.source,
					...
					s("startSession: transport initialized, starting session ", e);
					i.startSession(e).done(function (e) {
						s("startSession request returned ", e);
						s("startSession succeeded");
						h.resolve(new r({
								state : {
			...
			this.restoreSession = function (n) {
				var a,
				h,
				p,
				d;
				...
				if (!p || !p.sessionId) {
					a.reject(l({
							description : "restoreSession() failed to read state"
						}))
				} else {
					i = o(t.extend({}, n, {
								chatServerUrl : p.chatServerUrl
							}));
					d = {
						state : p,
						transport : i,
					...
					s("restoreSession: initializing transport");
					i.init().fail(c(a, s)).done(function () {
						s("restoreSession: transport initialized, restoring the session ", d);
						a.resolve(new r(d))
					})
				}
				return f(a)
		...
		function r(e) {
			...
			this.sendMessage = function (e) {
				...
				w("sending message ", r);
				g.sendMessage(r).fail(c(n, w)).done(function () {
					w("sending message done");
					n.resolve()
				});
				return f(n)
			};
			this.getTranscript = function (e) {
				...
				w("sending getTranscript ", n);
				g.getTranscript(n).fail(c(t, w)).done(function (e) {
					w("getTrasncript returned ", e);
					if (!e || !e.transcript) {
						t.reject(l({
		...
		function a(e) {
			throw new TypeError("Genesys Chat: " + e)
		}
		function f(e) {
			var t = e.promise();
			return {
	...
	/* Include: Include WebAPI/VCC REST Transport (if option selected) */
	function Transport_REST_WebAPI(e) {
		...
		function S(e) {
			...
			t.content = e.text ? {
			 ...
			 : undefined;
			t.party = {
				id : e.from ? parseInt(e.from.participantId) : "",
				type : e.from ? e.from.type : "",
				name : e.from ? e.from.nickname : ""
			};
			if (e.index >= 0 && e.index >= f) {
				f = e.index + 1
			}
			console.log("Received Index: " + t.index + ",  Type: " + t.type + ",  Content: " + e.text);
			return t
		}
		function x(e) {
			$.each(e.messages, function () {
				var e = S(this);
				if (e.type == "AgentConnected" && e.party.type == "External") {
					return false
				}
				if (d[e.type]) {
					d[e.type](e)
		...
		this.poll = function (e, t, n) {
			if (!m) {
				m = true;
				this.getTranscript().done(function (e) {
					x(e);
					m = false
				})
			}
		...
		this.startSession = function (n) {
			...
			return T("", "POST", {
				tenantName : i,
				nickname : e.formData.nickname || "Anonymous",
				firstName : e.formData.firstname || "John",
				lastName : e.formData.lastname || "Doe",
				emailAddress : e.formData.email || "",
				subject : e.formData.subject || "No Subject",
				text : "",
				userData : e.userData || {}
		...
		this.onAgentConnected = function (e) {
			d.AgentConnected = e
		};
		this.onAgentTyping = function (e) {
			d.AgentTyping = e
		};
		this.onAgentDisconnected = function (e) {
			d.AgentDisconnected = e
		};
		this.onMessageReceived = function (e) {
			d.MessageReceived = e
		};
		this.onSessionEnded = function (e) {
			d.SessionEnded = e
		};
	...
	/* Include: Include HTCC REST Transport (if option selected) */
	function Transport_REST_HTCC(e) {
		"use strict";
		function d(e) {
			var t = e.promise();
			return {
		...
		function m() {
			if (u.state() !== "resolved") {
				throw new Error("Cannot execute. Transport initialization is " + u.state())
			}
		}
		function g(e) {
			var t = {},
		...
		function b(e, s, o) {
			...
			$.ajax({
				...
				type : s,
				crossDomain : true,
				data : JSON.stringify(o, undefined, 2),
				headers : {
					"Content-Type" : "application/json"
				},
				xhrFields : {
				...
				success : function (e, t, n) {
					...
					if (n.getResponseHeader("X-CSRF-HEADER") && n.getResponseHeader("X-CSRF-TOKEN")) {
						console.log("setting new tokens");
						h.CSRFHeaderName = n.getResponseHeader("X-CSRF-HEADER");
						h.CSRFToken = n.getResponseHeader("X-CSRF-TOKEN")
					}
					u.resolve(e || {})
				},
				error : function (e) {
				...
				beforeSend : function (e) {
					if (h.CSRFHeaderName && h.CSRFToken) {
						e.setRequestHeader(h.CSRFHeaderName, h.CSRFToken);
						console.log("setting token headers");
						console.log(h)
					}
					if (r)
						e.setRequestHeader("ContactCenterId", r)
				}
			});
			return d(u)
		}
		function w(e) {
			console.log("RESTTransport.getToken()");
			var i = new t;
			p.href = n;
			$.ajax({
				url : p.protocol + "//" + p.host + "/api/v2/diagnostics/version",
				type : "GET",
				crossDomain : true,
				data : {},
				headers : {
				...
				success : function (t, n, r) {
					if (r.getResponseHeader("X-CSRF-HEADER") && r.getResponseHeader("X-CSRF-TOKEN")) {
						h.CSRFHeaderName = r.getResponseHeader("X-CSRF-HEADER");
						h.CSRFToken = r.getResponseHeader("X-CSRF-TOKEN");
						if (e) {
							e()
						}
					}
		...
		this.startSession = function () {
			...
			n = {};
			console.log(e);
			if (e.formData.firstname || e.formData.lastname || e.formData.email || e.userData.SkillName || e.userData.SkillLevel) {
				n = {
					FirstName : e.formData.firstname,
					LastName : e.formData.lastname,
					EmailAddress : e.formData.email,
					SkillName : e.userData.SkillName,
					SkillLevel : e.userData.SkillLevel
				}
			}
			return b("", "POST", {
	...
	/* Include: Include HTCC REST Apigee Transport (if option selected) */
	function Transport_REST_HTCC_APIGEE(e) {
		"use strict";
		function h(e) {
			var t = e.promise();
			return {
		...
		this.startSession = function () {
			console.log("RESTTransport.startSession()");
			var t = {};
			if (e.formData.firstname || e.formData.lastname || e.formData.email) {
				t = {
					FirstName : e.formData.firstname,
					LastName : e.formData.lastname,
					EmailAddress : e.formData.email
	...
	/* Include: Chat Widget UI */
	function GenesysWebChat(t) {
		function l() {
			var e = s;
			for (var i in e) {
				e[i] = $(e[i], r)
			...
			s.Title.text(t.title || "Chat Support")
		}
		function c() {
			o.sendMessage({
				message : s.Input.val(),
		...
		function h(e, t, n, r) {
			var i = $("<p><span class='name'></span></p>");
			i.addClass(r ? "system" : "").addClass(n ? "them" : "you");
			i.append(t);
			i.find(".name").text(e);
			s.Transcript.append(i);
			i.fadeIn();
			d()
		}
		function p(e, t) {
			var n = window.open(t);
			if (!n) {
				var r = $("<p class='notice them'><span class='name'></span><a href='' target='_BLANK'></a></p>");
				r.find(".name").text(e);
				r.find("a").text(t)[0].href = t;
				s.Transcript.append(r);
				r.fadeIn();
				d()
			}
		}
		...
		function v(e) {
			...
			case "pod":
				return Transport_REST_HTCC;
			case "apg":
				return Transport_REST_HTCC_APIGEE
			}
		}
		var n = this,
		t = t || {},
		r = $(t.template),
		i = $("<div class='end' title='End Chat'></div>"),
		s = {
			Title : ".title",
...
/* Stef TMP 
    setTimeout(function(){
    if (document.getElementById("genesys_video")!=null) {
        console.log("Enabling audio/video escalation "+document.getElementById("genesys_webchat_123").offsetHeight);
        var offset=document.getElementById("genesys_webchat_123").offsetHeight;
//        document.getElementById("genesys_video").style.bottom = offset+"px";
        document.getElementById("genesys_video").style.visibility = "inherit";
    }
},1500);
*/
					e.onMessageReceived(function (e) {
						...
						if (e.content.type == "notice") {
							p(e.party.name, e.content.text)
						} else {
							h(e.party.name, e.content.text, e.party.type.agent, n)
						}
					...
					e.onAgentConnected(function (e) {
						if (e.party.type == "Agent") {
                            ...
                            if (document.getElementById("genesys_video")!=null) {
                                console.log("Enabling audio/video escalation "+document.getElementById("genesys_webchat_123").offsetHeight);
                                //document.getElementById("genesys_video").style.bottom = document.getElementById("genesys_webchat_123").offsetHeight+"px";
                                if (e.party.name!=undefined && e.party.name.length>0) {
                                    //var agentTitle = e.party.name.substring(0, Math.max(e.party.name.indexOf(e.party.name),14));
                                    var agentTitle = e.party.name.substring(0, e.party.name.indexOf(" "));
                                    document.getElementById("videoTitle").textContent = "You can now talk with "+agentTitle;
                                }
                                console.log("agentTitle="+agentTitle);
                                document.getElementById("genesys_video").style.visibility = "inherit";
                            }
							h(e.party.name, (e.party.name || "Agent") + " Connected", false, true)
						} else if (e.party.type == "Client") {
							h(e.party.name, "Chat Started", false, true)
						}
					});
					e.onAgentDisconnected(function (e) {
						if (e.party.type == "Agent") {
							h(e.party.name, (e.party.name || "Agent") + " Disconnected", false, true)
						} else if (e.party.type == "Client") {
							h(e.party.name, "Chat Ended", false, true)
                            // SL
                            var evSL = document.createEvent('HTMLEvents');
                            evSL.initEvent("chat-stop", true, true);
                            document.dispatchEvent(evSL);
						}
						t.IsTyping.fadeOut()
					});
					e.onAgentTyping(function (e) {
						if (e.party.type == "Agent") {
							if (e.isTyping) {
								t.IsTyping.html((e.party.name || "Agent") + " is typing...").fadeIn()
							} else {
								t.IsTyping.fadeOut()
							}
						}
					...
					e.onSessionEnded(function (e) {
						h("", "Chat Ended", false, true);
						t.Input.addClass("disabled")[0].disabled = true;
						t.IsTyping.fadeOut();
						o = null
						// SL
						var evSL = document.createEvent('HTMLEvents');
					...
					o = e;
					t.Input.removeClass("disabled")[0].disabled = false;
					i.hide();
					i.click(function () {
						n.endSession();
						i.fadeOut()
					});
					t.Transcript.append(i).append("<br/>");
					i.fadeIn();
					$(window).unload(function () {
						n.endSession()
					})
				}).fail(function () {
					h(e.party.name, "Failed to Start Chat Session", false, true)
				})
			}
			return this
		};
		...
		this.checkForm = function () {
			...
			var e = s.Form,
			n = true,
			r = ["firstname", "lastname", "nickname", "subject", "email"],
			i = {
				firstname : false,
				lastname : false,
				nickname : false,
