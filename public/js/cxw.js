...
const CXW_WIDGET_URL = 'https://app.genesys.cloud/widgets/'+CXW_VERSION+'/widgets.min.js';
var disableWebSockets=false;
var _demoHost = '//core.demo.genesys.com'; 
var VIDYO_FILES = "//core.demo.genesys.com/vidyo";  

function saveChatTranscriptSfdc() {
    _genesys.widgets.bus.command('WebChatService.getTranscript').done(function(transcriptData){
        //console.log('transcript data:',transcriptData);
        transcript="";
        for (msg in transcriptData.messages) {
            msgDetail=transcriptData.messages[msg];
            //console.log('msg:',msgDetail);
            if (msgDetail.type=='Message') {
                    msgDetailText=msgDetail.text;
                    if (msgDetailText!==undefined && typeof msgDetailText.replace === 'function') {
                        //console.log(msgDetail.from.name);
                        transcript+=msgDetail.from.name + ": ";
                        //console.log(msgDetailText.replace(/(<([^>]+)>)/ig,""));
                        transcript+=msgDetailText.replace(/(<([^>]+)>)/ig,"");
                        //console.log("");
                        transcript+="\r\n\r\n";
                    }
            }
            console.log("Stef Chat Transcript: "+transcript);
        }
        duration=(transcriptData.messages[transcriptData.messages.length - 1].timestamp - transcriptData.messages[0].timestamp)/1000;
        console.log("Stef duration: "+duration);
        var sfdcSaveTranscript = {
            "async": true,
            "crossDomain": true,
            "url": "https://core.demo.genesys.com/cbws/SfdcCreateTaskForContactEmail",
            "method": "POST",
            "headers": {
            ...
            "data": JSON.stringify({
                ...
                "subject":"GInsurance Chat with Kate",
                "comments":transcript,
                "duration":duration,
                "interactionID":"",
                "disposition":"",
                "gDemoID":$.siteData.customer['customer.customer_phone'],
                "gDemoService":"GInsurance"
            })
        }

        $.ajax(sfdcSaveTranscript).done(function (response) {
          console.log("Stef Saving Chat Transcript in SFDC result:", response);
        });
    })
}
...
var widgetCX = (function(){
    ...
    var _initPureCloudCfg = function(){ 
        ...
        */
        ...
        window._genesys.widgets = {
            ...
            onReady: function(oCXBus){
                ...
                oWCC.subscribe("WebChat.completed", function(){
                    if (demoProcess.getParameterByName("demoid") === 'icarus') {
                        saveChatTranscriptSfdc();
                    }
                });
...
*/
   ...
   // public interface
    oPublished = {
        ...
        initPureCloudCfg: _initPureCloudCfg
    };
    ...
    if ($.siteData.authenticated) {
        if ($.siteData.settings['platform'] === 'purecloud') {
            ...
            widgetCX.widgetLoad(function(){
                gsolUser.customizePureCloudWidgets(simdomain_email);
            });
